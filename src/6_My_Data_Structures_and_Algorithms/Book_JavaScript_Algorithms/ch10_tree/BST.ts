...
class BinarySearchTree {
  ...
  /* 私有方法 */
  private insertNode(node: Node<number>, key: number) {
    //插入操作实体函数
    if (key > node.key) {
      //在右边插入
      if (node.right === null) {
  ...
  private getMinNode(node: Node<number>) {
    //得到最小的节点
  ...
  private getMaxNode(node: Node<number>) {
    //得到最大的节点
  ...
  private searchNode(node: Node<number>, key: number) {
    //根据键值查找节点 递归版
  ...
  private removeNode(node: Node<number>, key: number) {

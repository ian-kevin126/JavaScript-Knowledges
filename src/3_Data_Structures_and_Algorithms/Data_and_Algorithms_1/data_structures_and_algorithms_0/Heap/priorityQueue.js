...
class PriorityQueue {
  ...
  enqueue(
    value,
    priority // Time Complexity O(log n)
  ) {
    this.count++;
  ...
  bubbleDown() {
    ...
    while (true) {
      ...
      if (leftChildIndex < length) {
        // leftChildIndex is inbound, then assign the value to the left child
        leftChild = this.datas[leftChildIndex];

        if (leftChild.priority < element.priority) {
          // If leftChild is lesser than element then set swap equal to leftChildIndex
          swap = leftChildIndex;
        }
      }

      if (rightChildIndex < length) {
        // rightChildIndex is inbound, then assign the value to the right child

...
// 左右柯理化：柯理化把参数拆解了，上面的 curryAdd 是从左往右处理的，也就是从第一个参数开始，就是左柯理化。如果从右开始，就是右柯理化。如：
...
/**
 * 为什么要有柯理化：与其说为什么要有柯理化，不如说在 lambda 演算定义函数的时候，一个函数只能接收一个参数，这就决定了它必须通过某种技巧，让表达式去处理多个参数的运算，
 * 比如加法。所以，柯理化算是其中的一个技巧，通过返回函数，接收第二个参数，实现类似加法的操作。
